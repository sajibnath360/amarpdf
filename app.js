...
async function imagesToPdf(){
  const imgs=state.files.filter(f=>/^image\//i.test(f.type)||/\.(jpe?g|png|webp)$/i.test(f.name));
  ...
  const paper=$("#imgPaper")?.value||"fit";
  const margin=Math.max(0,Math.min(144,+$("#imgMargin")?.value||0));
  const A4=[595.28,841.89],LETTER=[612,792];
  const out=await PDFDocument.create();

  for(let i=0;i<imgs.length;i++){
    const file=imgs[i];
    setProcessingProgress("imagepdf",Math.round(i/imgs.length*90),`Adding image ${i+1} of ${imgs.length}…`);

    const source=await readImageSource(file);
    const imgW=Math.max(1,source.width),imgH=Math.max(1,source.height);
    const type=(file.type||"").toLowerCase();
    const ext=(file.name.split(".").pop()||"").toLowerCase();
    let embedded;

    // JPEG/PNG can be embedded directly. WebP/other browser-readable images
    // are rasterized to PNG because pdf-lib does not natively embed WebP.
    if(type==="image/jpeg"||type==="image/jpg"||ext==="jpg"||ext==="jpeg"){
      embedded=await out.embedJpg(await readBytes(file));
    }else if(type==="image/png"||ext==="png"){
      embedded=await out.embedPng(await readBytes(file));
    }else{
      const canvas=document.createElement("canvas");
      canvas.width=imgW;canvas.height=imgH;
      const ctx=canvas.getContext("2d");
      if(!ctx){source.close();throw new Error(`Could not prepare ${file.name}.`)}
      ctx.drawImage(source.source,0,0,imgW,imgH);
      const blob=await new Promise((resolve,reject)=>canvas.toBlob(b=>b?resolve(b):reject(new Error(`Could not encode ${file.name}.`)),"image/png"));
      embedded=await out.embedPng(new Uint8Array(await blob.arrayBuffer()));
    }
    source.close();

    let pageW,pageH;
    if(paper==="a4"){[pageW,pageH]=A4}
    else if(paper==="letter"){[pageW,pageH]=LETTER}
    else {pageW=imgW+margin*2;pageH=imgH+margin*2}
  ...
  setProcessingProgress("imagepdf",100,"Finalizing…");
  const bytes=await out.save({useObjectStreams:true,addDefaultPage:false});
  const base=imgs.length===1?imgs[0].name.replace(/\.[^.]+$/i,""):"images";
  downloadBytes(bytes,`${base}.pdf`,"application/pdf");
  setStatus(`Done — ${imgs.length} image${imgs.length>1?"s":""} converted to PDF.` ,"success");
}
